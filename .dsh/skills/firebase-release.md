...
description: Build and release the Android app to Firebase App Distribution, with optional version and release-notes arguments, then commit and push the version bump. Invoke as /firebase-release, optionally followed by a version and release notes text.
whenToUse: The user asks to release, bump, upload, or distribute the app to Firebase App Distribution (this repo is a WaniKani client — never release to any other destination).
---

Release the app to Firebase App Distribution. The user's message may contain arguments after
`/firebase-release` — both are optional:

- If the message contains a token that looks like a version (e.g. `v1.4`, `1.4`), use it as the
  new version.
- Any other free text in the user's message is release notes supplied by the user — use it
  verbatim instead of generating notes.

This repo distributes builds through the Firebase App Distribution Gradle plugin, configured in
`app/build.gradle.kts`. Only the `debug` build type has a `firebaseAppDistribution` block
(`groups = "friends"`), so that's the variant to build and upload — confirm this is still true by
checking the file rather than assuming, since it's the kind of thing that can change.
...
Unless release notes text was supplied in the user's message, generate them:
